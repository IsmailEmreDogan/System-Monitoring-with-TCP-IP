...
CpuServer::CpuServer(QObject *parent) : QObject(parent)
{
    startServer();
}

void CpuServer::startServer()
{
    serverSocket = new QTcpServer(this);
    if (!serverSocket->listen(QHostAddress::LocalHost, 12344)) {
        qDebug() << "Server could not start!";
        return;
    }
    qDebug() << "Server started!";
    connect(serverSocket, &QTcpServer::newConnection, this, &CpuServer::handleNewConnection);
}

void CpuServer::handleNewConnection()
{
    clientSocket = serverSocket->nextPendingConnection();
    qDebug() << "New client connected from" << clientSocket->peerAddress();
    connect(clientSocket, &QTcpSocket::readyRead, this, &CpuServer::readData);
    connect(clientSocket, &QTcpSocket::disconnected, clientSocket, &QTcpSocket::deleteLater);
}

uint16_t CpuServer::crc16(const QByteArray &data) {
...
QByteArray CpuServer::jsonToPacket(const QByteArray &jsonData)
{
...
void CpuServer::readData()
{
    ...
    while (startIndex != -1 && endIndex != -1 && endIndex > startIndex) {
        ...
        uint16_t receivedCRC = ((unsigned char)packet[packet.size() - 3] << 8) +
                               (unsigned char)packet[packet.size() - 2];
